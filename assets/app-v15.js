...
(function(){
  ...
  const T={
    it:{heroH:"Il tuo assistente per le videochiamate",heroP:"Ascolta la conversazione (colloquio, riunione, vendita…) e ti suggerisce cosa rispondere, in tempo reale. Con la tua chiave AI gratuita, tutto dentro il browser.",s1:"Scegli come funziona",s2:"Premi Ascolta",s3:"Leggi la risposta",
      ...
      proT:"Interview Sprint",proBadge:"prezzo fisso",proD:"Accedi: modelli e server UE, chiave nostra. Sblocco a prezzo fisso (19 €): mai una tariffa legata a colloqui o assunzioni.",
      acEmailL:"Email",acPassL:"Password (min 8 caratteri)",acLogin:"Accedi",acSignup:"Registrati",acLogout:"Esci",acUpgrade:"Interview Sprint — 19 € (prezzo fisso, mai in base all'assunzione)",acSync:"Ho pagato — aggiorna",proModelL:"Modello (dal nostro cloud)",
    ...
    en:{heroH:"Your assistant for video calls",heroP:"It listens to the conversation (interview, meeting, sales…) and suggests what to say, in real time. With your free AI key, all in your browser.",s1:"Pick how it works",s2:"Press Listen",s3:"Read the answer",
      ...
      proT:"Interview Sprint",proBadge:"fixed price",proD:"Sign in: EU models and servers, our key. Fixed-price unlock (€19): never a fee tied to interviews or hiring.",
      acEmailL:"Email",acPassL:"Password (min 8 chars)",acLogin:"Log in",acSignup:"Sign up",acLogout:"Log out",acUpgrade:"Interview Sprint — €19 (fixed price, never tied to hiring)",acSync:"I've paid — refresh",proModelL:"Model (from our cloud)",
  ...
  /* Claim onesti (revisione esterna): "100% locale" è riservato al client
     ...
     le precedenti. */
  const T8={
    it:{aiNotice:"🤖 Stai interagendo con un sistema di IA. Verifica sempre fatti, date, qualifiche ed esperienze prima di usarli.",sensitiveNote:"Non inserire dati sensibili (salute, religione, orientamento, opinioni politiche, sindacato…): non servono e non li chiediamo. Se li scrivi tu, restano cifrati e non vengono mai usati per valutarti o per addestrare modelli.",nextH:"Prossimi passi",next1:"1) Incolla un annuncio e carica il CV → Brief e Kit",next2:"2) Conferma 2–3 storie proposte dal CV",next3:"3) Prova il colloquio (simulazione IA)"},
    en:{aiNotice:"🤖 You are interacting with an AI system. Always verify facts, dates, qualifications and experiences before using them.",sensitiveNote:"Do not enter sensitive data (health, religion, orientation, political opinions, union…): we neither need nor ask for it. If you type it, it stays encrypted and is never used to rate you or to train models.",nextH:"Next steps",next1:"1) Paste an ad and upload your CV → Brief and Kit",next2:"2) Confirm 2–3 stories suggested from your CV",next3:"3) Rehearse the interview (AI simulation)"},
    zh:{aiNotice:"🤖 你正在与人工智能系统交互。使用前请核实事实、日期、资历与经历。",sensitiveNote:"请勿输入敏感数据（健康、宗教、性取向、政治观点、工会等）：我们不需要也不索取。若你自行输入，将加密保存，绝不用于评价你或训练模型。",nextH:"下一步",next1:"1) 粘贴职位并上传简历 → Brief 与申请包",next2:"2) 确认由简历建议的 2–3 个故事",next3:"3) 模拟面试（AI 模拟）"},
    es:{aiNotice:"🤖 Estás interactuando con un sistema de IA. Verifica siempre hechos, fechas, cualificaciones y experiencias antes de usarlos.",sensitiveNote:"No introduzcas datos sensibles (salud, religión, orientación, opiniones políticas, sindicato…): no los necesitamos ni los pedimos. Si los escribes, quedan cifrados y nunca se usan para evaluarte ni entrenar modelos.",nextH:"Próximos pasos",next1:"1) Pega un anuncio y sube tu CV → Brief y Kit",next2:"2) Confirma 2–3 historias sugeridas por tu CV",next3:"3) Ensaya la entrevista (simulación IA)"}
  };
  Object.keys(T8).forEach(l=>Object.assign(T[l],T8[l]));
  const T7={
  ...
  const RETURN_MAP={kit:'kit.html',storico:'storico.html',jobs:'jobs.html',practice:'practice.html',storie:'storie.html'};
  let RETURN_TO=null;
